...
class Map extends Component {
  ...
  async addMap() {
    ...
    const json = await d3.json("/api/counties"); 
  
    //Centering from stackoverflow thread.
    var center = d3.geoCentroid(json)	//GeoJSON
    var scale = 1;
    var offset = [width / 2, height / 5];
    var projection = d3.geoMercator().scale(1.1).center(center)
      .translate(offset);
    ...
    counties_main_group.selectAll("path")
      ...
      .attr("fill", function (d) {
        ...
        if (lookup[d.properties.k_id] < 0) {
          ...
          var blue = 0;
          if (red < 50) {
            blue = 255 - ((Math.abs(lookup[d.properties.k_id]) / var_min) * 255);
          }
