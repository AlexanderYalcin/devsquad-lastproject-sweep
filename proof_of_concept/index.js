...
async function printer(id, countyName) {
  let result;
  await fetch('http://localhost:3000/test')
    .then(response => response.json())
    .then(response => result = response)

  // console.log(result['0114']);
  // let queryCounty = function clickedCounty(event.target) {
  // id from the on hover function
  // }
  // into result `id`
  const exData = result[id];

  const xData = [];
  const yData = [];

  // var result2 = Object.keys(exData).map(function (key) {
  //   return { [key]: exData[key] };
  // });
  // console.log(result2);

  for (let el of exData) {
    xData.push(el.year);
    yData.push(el.value);
...
window.on_hoover = on_hoover;
function on_click_kommun(id, countyName) {
  console.log("accessed id: ", countyName);
  printer(id, countyName);
}
window.on_click_kommun = on_click_kommun;
// const exData = result['0114']
...
d3.json("GeoJSON/swedish_counties.geojson", function (json) {
  ...
  var scale = 1;
  var offset = [width / 2, height / 5];
  var projection = d3.geo.mercator().scale(1.1).center(center)
    .translate(offset);
  ...
  counties_main_group.selectAll("path")
    ...
    .attr("onclick", function (d) { return "on_click_kommun('" +  d.properties.k_id + "','" + d.properties.name + "')"; });
});
